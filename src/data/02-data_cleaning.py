...
def import_demographics(year_dir_path):
    ...
    dems = demographics[
        ["SEQN", "RIAGENDR", "RIDAGEMN", "RIDAGEYR", "RIDEXPRG", "RIDRETH1"]
    ].copy()
    dems.set_axis(
        [
            "id",
    ...
    dems = dems.astype({"id": "int", "sex": "int", "race": "int"})
...
def import_body_data(body_data_dir_path):
    ...
    measurements.set_axis(
        ["id", "weight_kg", "height_cm", "waist_circum_cm"], axis=1, inplace=True
    )
    ...
    measurements = measurements[
        measurements["weight_kg"].notna() & measurements["height_cm"].notna()
    ]
    ...
    smoker.set_axis(["id", "has_smoked", "is_smoker"], axis=1, inplace=True)
...
def import_predictors():
    ...
    predictors = pd.concat(
        [predictors99, predictors01, predictors03, predictors05], axis=0
    )
...
predictors_smoking.loc[
    (predictors_smoking["is_smoker"] == 1) | (predictors_smoking["is_smoker"] == 2),
    "smoker_status",
] = 1
